...
We included "Speaker Notes" in the [Google Slide](https://docs.google.com/presentation/d/1M9gE1Wv08GLSOgKtwCtLHypvGdmYd1BzGTxuklBDWRo/edit?usp=sharing), and the recording of our presentation [rehearsal](https://github.com/kobertlam/Energy_ETF_RYE_Forecast/blob/main/rehersal.mp4).
...
After acquiring our datasets, we have imported them into our Jupyter Notebook [master.ipynb](master.ipynb), conducted some preprocessing and completed a preliminary analysis. 
...
2. Is there any relationship between the crude oil prices and the ETF prices? Can we forecast the future ETF price based on both the historical ETF prices and the historical crude oil prices?
    ...
    - Using [R Statistics Package](https://github.com/kobertlam/Energy_ETF_RYE_Forecast/blob/machine_learning_model/correlation_R.R) we have noticed a strong correlation between the closing price of Oil and RYE ETF ([R=0.69](../machine_learning_model//Resources/correlation_RYE_Oil.png)) within the period of our analysis, confirming our initial assumption that the movement of oil prices could help to forecast the energy ETF prices.
...
Due to the fluctuation of the ETF prices and lack of strong seasonality, the time series ARIMA model wasn't able to provide an accurate price forecast. The result shows the average price of previous values that were used as future prediction plotted as a stright line. 
